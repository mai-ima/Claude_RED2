...
前線モードは色変数だけでなく **`assets/css/theme-endfield.css`** による専用スキンを
併せ持つ(手書き。全ページに読み込むが `[data-theme="endfield"]` スコープで、選択時
のみ適用され他テーマには一切影響しない)。ボタン(黄フラットの角切りピル+黒文字)・
カード(HUD隅ブラケット+黄ヘアライン)・見出し(「//」前置)・背景(トポ地紋+
斜めハッチ+微グリッド)・スクロールバー・フォーカスなどを作品意匠へ寄せ、選択すると
サイト全体がコラボ特設ページのように見える。他テーマは従来どおり色変数のみで動く。
...
- **第2弾の発表済み枠(H-2)**: 発表フルLPは作品名スラッグの特設URL `/collab/zzz/`・`/collab/hsr/`(第1弾と同方式)
  ...
  (`main.js`)。発表前のティザーは `/collab/wave2{,-2}/teaser/` に**保存版アーカイブ**
  (終了カウントダウン+当時のヒント・noindex)として残り、専用ページ末尾の小ボタンから辿れる。
  製品画像は**本レンダリング**(`svg_kudo`/`svg_seiki`。既存製品と同じ
  `assets/img/products/{{id}}-{{i}}.svg` 命名でカラバリ3色+`-front` 正面を生成)。
  仮デザインSVG(`svg_prototype`・不透明背景焼き込み)はSVGギャラリーにアーカイブ保管。
  LP構成は第1弾LPと同型(ヒーロー両面+タグ → 世界観/用語 → カラバリ実写 → 専用SoC →
  専用冷却 → 特徴 → 音響 → 開発の声 → 開発ログ → 仕様(第一報)/vs SUZAKU 4 →
  アクセサリ → 同梱物 → 予約手順 → スケジュール → FAQ)で、本文は第1弾最大超え
- **製品別FAQ**: コラボ4機種・コラボアクセサリ12種・旗艦(SUZAKU 4 / Pad 2)は
  `data_products.py` の `PRODUCT_FAQ` が共通のライン別FAQより優先して表示される
- **コラボタブレット(二状態ページ)**: `/collab/{slug}/tablet/`。同一URLに予告ステージ
  (カウントダウン)と発表ステージ(フルLP: 価格・スペック・スケジュール)を持ち、
  `reveal_at` と実時刻の判定で **ゼロ到達の瞬間に自動でフルLPへ切り替わる**
  (`data-reveal-stage` / `collab-core.js`)。`<title>`/description は発表前情報のみで、
  価格漏れは `selftest.py` が機械検査、切替動作は `interact.js` が実地テストする。
  カウントダウンには予告開始→発表の**進捗リング**と最終24時間の強調表示付き
